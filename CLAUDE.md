...
Print/cut registration: `uret_baski_svg`, `uret_kalip_svg`, and the DXF `HIZA` layer all carry the same four corner fiducials (via `hiza_isaretleri()`), placed at `(5,5),(W-5,5),(5,H-5),(W-5,H-5)` — outside the rounded frame, in the square-corner waste that the cut discards, so they never show on the toy. The template (kalıp) PDF stays a fixed **320×180** — it is the physical placement jig for the product, so its size must not change. The print PDF is **324×184** with the design centered (design center = page center = 162×92), so when the print is centered on the 320×180 template it overhangs an equal 2 mm bleed on every edge. Register by centering print-on-template (and matching the four marks); the geometry already resolves to one 320×180 trim, so residual print-shifted-relative-to-cut drift is a production-side scaling/positioning error, not a file error.
