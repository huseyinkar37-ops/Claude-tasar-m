...
UCAK_DX = 50.0   # uçağın gökyüzünde sağa kaydırma miktarı (helikopter iptal edilince)
...
def sahne_svg(yuva_goster=False):
    ...
    for sx, sy, sw in [(108, 9, 60), (238, 14, 46), (25, 22, 40)]:
...
def arac_detaylari():
    ...
    # ============ UÇAK ============
    # detaylar yerel (kaydırmasız) koordinatlarda çizilir, grup ile taşınır
    u = s_tasi(P["ucak"], -UCAK_DX, 0)
    e.append(f'<g transform="translate({UCAK_DX},0)">')
    ku = _klip(e, "kucak", u)
...
def uret_kalip_svg():
    """UV hizalama kalıbı: dış çerçeve + 4 köşe hiza haçı. Baskıyla BİREBİR
    çakışsın diye baskıyla aynı sayfa (324x184) ve 2 mm bleed ofseti kullanır."""
    icerik = [f'<g transform="translate({BLEED},{BLEED})">',
